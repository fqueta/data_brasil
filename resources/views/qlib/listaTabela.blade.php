...
<table class="table table-hover table-striped dataTable {{$routa}}" style="{{@$style}}">
    ...
    <tbody>
        @if(isset($dados))
            ...
            <tr ondblclick="window.location='{{ route($routa.'.edit',['id'=>$val->id]) }}?redirect={{urlencode($redirect.'idCad='.$val->id)}}'"  id="tr_{{$val->id}}" class="@if (isset($_GET['idCad']) && $_GET['idCad']==$val->id) table-info @endif" title="DÊ DOIS CLIQUES PARA ABRIR">
                    <td>
                    ...
                    <td class="text-right d-flex d-print-none">
                        @can('update',$routa)
                        @if ($routa=='familias')
                            <a href=" {{ route($routa.'.show',['id'=>$val->id]) }}?redirect={{urlencode($redirect.'idCad='.$val->id)}} " title="visualizar" class="btn btn-sm btn-outline-secondary mr-2">
                                <i class="fas fa-eye"></i>
                            </a>
                        @endif
                        <a href=" {{ route($routa.'.edit',['id'=>$val->id]) }}?redirect={{urlencode($redirect.'idCad='.$val->id)}} " title="Editar" class="btn btn-sm btn-outline-secondary mr-2">
                        ...
                        @else
                        <a href=" {{ route($routa.'.edit',['id'=>$val->id]) }}?redirect={{urlencode($redirect.'idCad='.$val->id)}} " class="btn btn-sm btn-outline-primary mr-2" title="Visualizar">
